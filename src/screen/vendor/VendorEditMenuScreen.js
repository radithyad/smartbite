...
export default function VendorEditMenuScreen({ route, navigation }) {
  ...
  const [emoji, setEmoji] = useState(menu.emoji || '🍽️');
  const [fotoUrl, setFotoUrl] = useState(menu.foto_url || null);
  const [tersedia, setTersedia] = useState(menu.tersedia ?? true);
  ...
  const ambilFoto = async (source) => {
    ...
    try {
      ...
      const fileName = `menu_${menu.toko_id}_${menu.id}.jpg`;
  ...
  const handleSimpan = async () => {
    ...
    Alert.alert('Berhasil! 🎉', 'Menu berhasil diperbarui.', [{ text: 'OK', onPress: () => navigation.goBack() }]);
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.inputWrapper}>
            ...
            <TextInput style={styles.input} placeholder="Nama menu" placeholderTextColor="#aaa" value={nama} onChangeText={setNama} />
          </View>

          <View style={styles.inputWrapper}>
            <Text style={styles.inputLabel}>Deskripsi</Text>
            <TextInput style={[styles.input, styles.inputMultiline]} placeholder="Deskripsi singkat..." placeholderTextColor="#aaa" value={deskripsi} onChangeText={setDeskripsi} multiline numberOfLines={3} />
          </View>

          <View style={styles.inputWrapper}>
            <Text style={styles.inputLabel}>Harga (Rp) *</Text>
            <TextInput style={styles.input} placeholder="15000" placeholderTextColor="#aaa" value={harga} onChangeText={setHarga} keyboardType="number-pad" />
          </View>
...
const styles = StyleSheet.create({
  ...
  fotoLabel: { fontSize: 14, fontWeight: '600', color: '#1565C0' },
